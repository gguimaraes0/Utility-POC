...
WHERE (habilidade:Habilidade {nome:'Análise de circuitos elétricos'})
CREATE (cd)-[:REQUER]->(habilidade)

// Vincula a CD à habilidade
MATCH (cd:CD {nome:'C.1.10'})
WHERE (habilidade:Habilidade {nome:'Eletrônica digital'})
CREATE (cd)-[:REQUER]->(habilidade)

// Vincula a CD à habilidade
MATCH (cd:CD {nome:'C.1.10'})
WHERE (habilidade:Habilidade {nome:'Eletrônica geral'})
CREATE (cd)-[:REQUER]->(habilidade)
