...
WHERE (habilidade:Habilidade {nome:'Custo computacional e complexidade de algoritmos'})
CREATE (cd)-[:REQUER]->(habilidade)

MATCH (cd:CD {nome:'C.1.2'})
WHERE (habilidade:Habilidade {nome:'Linguagens formais e autômatos'})
CREATE (cd)-[:REQUER]->(habilidade)

MATCH (cd:CD {nome:'C.1.2'})
WHERE (habilidade:Habilidade {nome:'Lógica básica'})
CREATE (cd)-[:REQUER]->(habilidade)

MATCH (cd:CD {nome:'C.1.2'})
WHERE (habilidade:Habilidade {nome:'Indução'})
CREATE (cd)-[:REQUER]->(habilidade)
